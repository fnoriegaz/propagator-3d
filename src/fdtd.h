...
inline __device__ real_t compute_stencil_backward_z(real_t *front, real_t *behind, real_t delta){
...
inline HOSTDEVICE float s_compute_stencil_backward(real_t *field, integer_t idx, integer_t stride, real_t delta){
	float output = (coeff[0] * (field[idx] - field[idx-1*stride]) -
			coeff[1] * (field[idx+1*stride] - field[idx-2*stride]) +
			coeff[2] * (field[idx+2*stride] - field[idx-3*stride]) -
			coeff[3] * (field[idx+3*stride] - field[idx-4*stride])) / delta;
